...
def build_config(run_name: str, model_name: str, epochs: int, peak_lr: float, global_batch_size: int, weight_decay: float, rank_batch_size: int, overrides: List[str], include_instance_metadata: Optional[bool] = False) -> ExperimentConfig:
    ...
    with open(f'/home/morg/students/gottesman3/knowledge-analysis-suite/OLMo-core/chunk_ordering_data/swapping_dict_interval_{swap_interval}.pkl', 'rb') as f:
        swapping_dict = pickle.load(f)  
        swapping_dict = {int(k): int(v) for k, v in swapping_dict.items()}  
    ...
    save_folder = Path(run_name, f"{model_name}_{optim_config.lr}_{data_loader_config.global_batch_size}_{weight_decay}_{max_duration.value}_{swap_interval}")
...
def main(run_name: str, model_name: str, epochs: int, peak_lr: float, global_batch_size: int, weight_decay: float, rank_batch_size: int, swap_interval: int, overrides: List[str]):
...
if __name__ == "__main__":
    ...
    run_name, model_name, epochs, peak_lr, global_batch_size, weight_decay, rank_batch_size, swap_interval, *overrides = sys.argv[1:]
    epochs = int(epochs)
    ...
    print(f"Run name: {run_name} epochs {epochs} peak_lr {peak_lr} global_batch_size {global_batch_size} weight_decay {weight_decay} rank_batch_size {rank_batch_size} interval {swap_interval} overrides {overrides}", flush=True)
    prepare_training_environment()
    try:
        main(run_name, model_name, epochs, peak_lr, global_batch_size, weight_decay, rank_batch_size, swap_interval, overrides)
    finally:
        teardown_training_environment()